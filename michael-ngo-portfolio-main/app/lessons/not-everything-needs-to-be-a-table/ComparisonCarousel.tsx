...
const examples = [
  ...
  {title:"Quality assurance",lesson:"The table records activities. The cycle shows follow-through.",headers:["Activity","Purpose"],rows:[["Inspect","Review work"],["Identify","Record issue"],["Correct","Resolve gap"],["Verify","Confirm result"],["Improve","Prevent recurrence"]],question:"What are the QA activities?",meaning:"How does an issue become an improvement?",type:"cycle"},
...
export default function ComparisonCarousel(){
  ...
  return <div className={`comparison-carousel carousel-theme-${item.type}`} aria-roledescription="carousel" aria-label="Table and graphic comparisons">
...
function Graphic({type}:{type:string}){
  ...
  if(type==="cycle") return <div className="carousel-graphic qa-loop">
    <strong>QA</strong>
    <div className="qa-node qa-one"><b>1</b><span>Inspect</span></div>
    <div className="qa-node qa-two"><b>2</b><span>Identify</span></div>
    <div className="qa-node qa-three"><b>3</b><span>Correct</span></div>
    <div className="qa-node qa-four"><b>4</b><span>Verify</span></div>
    <div className="qa-node qa-five"><b>5</b><span>Improve</span></div>
    <i className="qa-arrow arrow-one">→</i><i className="qa-arrow arrow-two">↓</i><i className="qa-arrow arrow-three">←</i><i className="qa-arrow arrow-four">←</i><i className="qa-arrow arrow-five">↑</i>
  </div>;
